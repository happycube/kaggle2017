...
DATA_PATH = os.path.join(this_dir, "../../twosigma/input", "train.h5")
...
class Environment(object):
    def __init__(self):
        with pd.HDFStore(DATA_PATH, "r") as hfdata:
            ...
            self.full = self.test.loc[:, ['timestamp', 'y', 'id']]
            self.full['y_hat'] = 0.0
            self.temp_test_y = None
            
        self.fname = None # 'run' + time.strftime('%m%d%H%m%s')
        self.localrun = True

        self.ID_COL_NAME = 'id'
        self.SAMPLE_COL_NAME = 'sample'
        self.TARGET_COL_NAME = 'y'
        self.IIME_COL_NAME = 'timestamp'
    ...
    def step(self, target):
        ...
        if self.unique_idx == self.n:
            ...
            if self.fname is None:
                self.fname = 'exp-{0}'.format(int(score * 10000000))
            
            self.full.to_pickle(self.fname + '.pkl')
        else:
            reward = r_score(self.temp_test_y, target.loc[:, 'y'])
            done = False
            info = {}
